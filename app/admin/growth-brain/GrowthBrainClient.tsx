...
export default function GrowthBrainClient() {
    ...
    const generateCoverImage = useCallback(async () => {
        ...
        const topic = selectedIdea?.title || '';
        const hook = generatedContent?.hook || '';
        setCoverImageLoading(true);
        try {
            const res = await fetch('/api/growth-brain/image', {
                method: 'POST',
                                ...
                                {generatedContent && (
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                        ...
                                        <div style={{ borderTop: '1px solid #1a1a2e', paddingTop: 16 }}>
                                            <div style={{ fontSize: 11, color: '#888', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Imagen de Portada (9:16)</div>
                                            {!coverImageUrl && (
                                                <button
                                                    onClick={generateCoverImage}
                                                    disabled={coverImageLoading}
                                                    style={{
                                                        width: '100%', padding: '10px', borderRadius: 10, border: '1px solid rgba(0,229,255,0.25)',
                                                        background: coverImageLoading ? 'rgba(0,229,255,0.04)' : 'rgba(0,229,255,0.08)',
                                                        color: coverImageLoading ? 'rgba(0,229,255,0.4)' : '#00E5FF',
                                                        fontSize: 13, fontWeight: 600, cursor: coverImageLoading ? 'not-allowed' : 'pointer',
                                            ...
                                            {coverImageUrl && (
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                                    <img src={coverImageUrl} alt="Cover" style={{ width: '100%', maxWidth: 220, borderRadius: 10, border: '1px solid #222', display: 'block', margin: '0 auto' }} />
                                                    <div style={{ display: 'flex', gap: 6 }}>
                                                        <a href={coverImageUrl} download="cover.jpg" target="_blank" rel="noreferrer"
                                                            style={{ flex: 1, padding: '8px', borderRadius: 8, background: 'rgba(0,200,100,0.08)', border: '1px solid rgba(0,200,100,0.2)', color: '#00c864', fontSize: 12, fontWeight: 600, textAlign: 'center', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                                                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                                                            Descargar
                                                        </a>
                                                        <button onClick={generateCoverImage} disabled={coverImageLoading}
                                                            style={{ flex: 1, padding: '8px', borderRadius: 8, background: 'rgba(0,229,255,0.06)', border: '1px solid rgba(0,229,255,0.15)', color: '#00E5FF', fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontFamily: 'inherit' }}>
